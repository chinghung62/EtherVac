...
<script type="text/javascript">
			...
			if (!pattern.test(d["address"])) {
				...
				if (!pattern.test(d["email"])) {
					...
					await connectWallet().then(async (address) => {
						getNonce();
						let message = "Welcome to EtherVac!\n\nYou're about to add a new admin.\n\nWallet address:\n"
								+ address
								+ "\n\nNonce:\n"
								+ nonce;
					...
					if (d["signature"] != null) {
						...
						XHRequest("addAdmin", JSON.stringify(d), {callback: "afterAdd"});
						return;
					}
				}
			}
	...
	function afterAdd(rc = null) {
		if (rc != null) {
			if (rc["ok"] === true) {
				$e("span-message").innerHTML = "A new admin has been added.";
				loadTable();
				reset();
			}
		}
	...
	async function update(element, count, address, name, phoneNo, email) {
		...
		if (d["count"] == "") {
			...
			if (!pattern.test(d["address"])) {
				...
				if (!pattern.test(d["email"])) {
					...
					if (d["signature"] != null) {
						$e("span-message").innerHTML = "Please wait...";
						XHRequest("updateAdmin", JSON.stringify(d), {callback: "afterUpdate"});
						return;
					}
				}
			}
	...
	function afterUpdate(rc = null) {
		if (rc != null) {
			if (rc["ok"] === true) {
				$e("span-message").innerHTML = "The admin has been updated.";
				loadTable();
			}
		}
	...
	async function remove(count, address) {
		...
		if (d["count"] == "") {
			...
			await connectWallet().then(async (address) => {
				getNonce();
				let message = "Welcome to EtherVac!\n\nYou're about to delete an existing admin.\n\nWallet address:\n"
						+ address
						+ "\n\nNonce:\n"
						+ nonce;
			...
			if (d["signature"] != null) {
				$e("span-message").innerHTML = "Please wait...";
				XHRequest("deleteAdmin", JSON.stringify(d), {callback: "afterRemove"});
				return;
			}
		}
	...
	function afterRemove(rc = null) {
		if (rc != null) {
			if (rc["ok"] === true) {
				$e("span-message").innerHTML = "The admin has been deleted.";
				loadTable();
			}
		}
...
<body onload="loadUserInfo(); loadTable();">
	...
	<div class="body">
		<div class="left-sidebar">
			<ul>
				<li><a class="selected" href="register-admin.jsp">Admin Registration</a></li>
			</ul>
			<ul>
