...
<script type="text/javascript">
	...
	function getAdminInfo(rc = null) {
		if (rc == null) {
			XHRequest("getAdminInfo", JSON.stringify({}), {callback: "getAdminInfo"});
		} else {
		...
		if (d["name"] == "") {
			...
			if (!pattern.test(d["email"])) {
				...
				await connectWallet().then(async (address) => {
					getNonce();
					let message = "Welcome to EtherVac!\n\nYou're about to update your info (current admin).\n\nWallet address:\n"
							+ address
							+ "\n\nNonce:\n"
							+ nonce;
				...
				if (d["signature"] != null) {
					...
					XHRequest("updateAdminInfo", JSON.stringify(d), {callback: "afterUpdateAdminInfo"});
					return;
				}
			}
		}

		afterUpdateAdminInfo();
	}

	function afterUpdateAdminInfo(rc = null) {
		if (rc != null) {
			if (rc["ok"] === true) {
				$e("span-message").innerHTML = "Admin profile has been updated.";
				cancelEdit();
				getAdminInfo();
			}
		}
	...
	function edit() {
		...
		button.setAttribute("onclick", "updateAdminInfo();");
...
<body onload="loadUserInfo(); getAdminInfo();">
	<div class="header">
		<div class="logo">
