...
if (validate) {
	...
	} else if (!d.containsKey("location")) { // validate parameter 'location'
		rc.put("error_code", 400);
		rc.put("description", "Bad Request: Parameter 'location' is required");
	} else if (d.get("location").equals("")) {
		rc.put("error_code", 400);
		rc.put("description", "Bad Request: 'location' can't be empty");
	} else if (((String) d.get("location")).length() > 200) {
		rc.put("error_code", 400);
		rc.put("description", "Bad Request: 'location' length can't be more than 200");
...
if (execute) {
	...
	if (!admin.isExist()) {
		if (!clinic.isExist()) {
	boolean ok = u_admin.addClinic((String) d.get("address"), (String) d.get("name"),
	(String) d.get("phone_no"), (String) d.get("email"), (String) d.get("location"));
	...
	rc.put("error_code", 400);
	rc.put("message", "The clinic already exists.");
	rc.put("description", "Bad Request: The clinic already exists");
		}
	} else {
		rc.put("error_code", 400);
		rc.put("message", "The address already exists as an admin.");
		rc.put("description", "Bad Request: The address already exists as an admin");
	}
		} else {
	rc.put("error_code", 500);
	rc.put("description", "Internal Server Error: Smart Contract Error");
