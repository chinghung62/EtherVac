...
if (validate) {
	...
	rc.put("description", "Bad Request: 'barcode' length can't be more than 13");
		} else if (!d.containsKey("status")) { // validate parameter 'status'
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: Parameter 'status' is required");
		} else if (d.get("status").equals("")) {
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: 'status' can't be empty");
...
if (execute) {
	...
	(String) session.getAttribute("address"))) {
		...
		if (vaccine != null) {
	if (vaccine.isExist()) {
		boolean ok = u_admin.updateVaccineStatus(Integer.parseUnsignedInt((String) d.get("count")),
				(String) d.get("barcode"), Boolean.parseBoolean((String) d.get("status")));
		...
		rc.put("error_code", 400);
		rc.put("message", "The vaccine doesn't exist.");
		rc.put("description", "Bad Request: The vaccine doesn't exist");
	}
		} else {
	rc.put("error_code", 500);
	rc.put("description", "Internal Server Error: Smart Contract Error");
