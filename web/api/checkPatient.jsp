...
if (execute) {
	...
	Patient patient = u_clinic.getPatient((String) d.get("address"));

	if (patient != null) {
		if (patient.isExist()) {
	rc.put("address", patient.getAddress());
	rc.put("ic_no", patient.getIcNo());
	rc.put("name", patient.getName());
	rc.put("gender", patient.getGender());
	rc.put("nationality", patient.getNationality());
	rc.put("ok", true);
		} else {
	rc.put("error_code", 400);
	rc.put("message", "Patient not found.");
	rc.put("description", "Bad Request: The patient doesn't exist");
		}
	} else {
