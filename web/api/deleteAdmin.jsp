...
if (execute) {
	...
	(String) session.getAttribute("address"))) {
		...
		if (Integer.parseUnsignedInt((String) d.get("count")) != 0) {
	Admin u_admin = new Admin((String) session.getAttribute("address"));
	Admin admin = u_admin.getAdmin((String) d.get("address"));

	if (admin != null) {
		if (admin.isExist()) {
			if (!((String) d.get("address")).equals(session.getAttribute("address"))) {
				boolean ok = u_admin.deleteAdmin(Integer.parseUnsignedInt((String) d.get("count")),
	...
	rc.put("error_code", 400);
	rc.put("message", "The first admin (system owner) cannot be deleted.");
	rc.put("description", "Bad Request: Cannot delete the first admin (system owner)");
		}
	}
