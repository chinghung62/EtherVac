...
if (execute) {
	...
	HashMap<String, Object> clinicDict;

	Admin u_admin = new Admin((String) session.getAttribute("address"));
	ArrayList<Clinic> clinics = u_admin.getAllClinics();

	if (clinics != null) {
		for (Clinic clinic : clinics) {
	clinicDict = new HashMap<String, Object>();
	clinicDict.put("count", clinic.getCount());
	clinicDict.put("address", clinic.getAddress());
	clinicDict.put("name", clinic.getName());
	clinicDict.put("phone_no", clinic.getPhoneNo());
	clinicDict.put("email", clinic.getEmail());
	clinicDict.put("location", clinic.getLocation());
	result.add(clinicDict);
		}
