...
if (validate) {
	...
	rc.put("description", "Bad Request: 'id' is out of range");
		} else if (!d.containsKey("clinic_signature")) { // validate parameter 'clinic_signature'
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: Parameter 'clinic_signature' is required");
		} else if (d.get("clinic_signature").equals("")) {
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: 'clinic_signature' can't be empty");
		} else if (!User.verifySignature((String) d.get("clinic_signature"))) {
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: Invalid value for 'clinic_signature'");
...
if (execute) {
	...
	rc.put("description", "Bad Request: Invalid message signature");
		} else if (!EthereumSignature.verifySignature((String) d.get("clinic_signature"), certificateMessage,
		(String) session.getAttribute("address"))) {
	rc.put("error_code", 400);
	...
	if (certificate.getClinicSignature().equals("")) {
		if (!certificate.getPatientSignature().equals("")) {
			boolean ok = u_clinic.signCertificate(Integer.parseUnsignedInt((String) d.get("id")),
					(String) d.get("clinic_signature"));
			...
			rc.put("error_code", 400);
			rc.put("message", "The vaccination haven't signed by patient.");
			rc.put("description", "Bad Request: The vaccination haven't signed by patient");
		}
	} else {
		rc.put("error_code", 400);
