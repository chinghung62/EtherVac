...
if (validate) {
	...
	rc.put("description", "Bad Request: 'purpose' can't be empty");
		} else if (((String) d.get("purpose")).length() > 100) {
	rc.put("error_code", 400);
	rc.put("description", "Bad Request: 'purpose' length can't be more than 100");
...
if (execute) {
	...
	(String) session.getAttribute("address"))) {
		...
		if (vaccine != null) {
	if (vaccine.isExist()) {
		boolean ok = u_admin.updateVaccine(Integer.parseUnsignedInt((String) d.get("count")),
				(String) d.get("barcode"), (String) d.get("name"), (String) d.get("purpose"),
				(String) d.get("manufacturer"));
